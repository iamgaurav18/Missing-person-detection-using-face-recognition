...
def basicPageSetup(pageNo):
    ...
    right_frame = tk.LabelFrame(content, text="Detected Person", bg="#202d42", font="Arial 20 bold", bd=4,
...
def getPage1():
    ...
    basicPageSetup(1)
    heading.configure(text="Register Person")
    ...
    input_fields = ("Name", "Father's Name", "Mother's Name", "Gender", "DOB(yyyy-mm-dd)", "Blood Group",
                    "Identification Mark", "Nationality", "Religion", "Charateristics", "Profile Image")
...
def showCriminalProfile(name):
    ...
    top.title("Person Profile")
    top.geometry("1500x900+%d+%d"%(root.winfo_x()+10, root.winfo_y()+10))

    tk.Label(top, text="Person Profile", fg="white", bg="#202d42", font="Arial 20 bold", pady=10).pack()
...
def startRecognition():
    ...
    if (len(face_coords) == 0):
        ...
        if (len(recognized) == 0):
            messagebox.showerror("Error", "Person not recognized.")
            return
...
def getPage2():
    ...
    basicPageSetup(2)
    heading.configure(text="Detect Person")
    right_frame.configure(text="Detected Person")
...
def getPage3():
    ...
    right_frame.configure(text="Detect Persons")
...
######################################## Home Page ####################################
tk.Label(pages[0], text="Missing Person Detection System", fg="white", bg="#202d42",
...
tk.Button(btn_frame, text="Register Person", command=getPage1)
tk.Button(btn_frame, text="Video Surveillance", command=getPage3)
tk.Button(btn_frame, text="Detect Person", command=getPage2)
